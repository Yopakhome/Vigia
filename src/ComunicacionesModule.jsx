...
import { Upload, FileText, AlertTriangle, Clock, ChevronRight, Zap, MessageSquare, ArrowDown, ArrowUp, HelpCircle, Link2, Eye, RefreshCw, X, Check, Mail, File, Package } from "lucide-react";
...
const SEED_COMMS = [
  { id:"comm1", original_name:"Auto_Seguimiento_034_2026_CRA.pdf", file_type:"pdf", direction:"entrante", comm_type:"auto_seguimiento", comm_date:"2026-03-28", radicado_number:"2026-034-CRA", authority_name:"AUTORIDAD COMPETENTE - Regional", subject_extracted:"Requerimiento de subsanacion ICA Semestral - Res. 786/2016", analysis_status:"confirmado", overall_confidence_pct:97, linked_instrument_id:"c1000000-0000-0000-0000-000000000001", impact_types:["modifica_plazo","requiere_informacion"], impact_urgency:"critica", impact_summary:"Requiere subsanar OBL-04 en 15 dias habiles. Plazo actualizado al 18 de abril de 2026.", actions:[{type:"update_due_date",desc:"OBL-04 - Nueva fecha limite: 18 abr 2026"}] },
  { id:"comm2", original_name:"Respuesta_ICA_S2_2025_enviada.pdf", file_type:"pdf", direction:"saliente", comm_type:"respuesta", comm_date:"2026-04-10", radicado_number:"2026-1847-TITULAR", authority_name:"AUTORIDAD COMPETENTE - Regional", subject_extracted:"Radicacion ICA Segundo Semestre 2025 - AS I Baranoa", analysis_status:"confirmado", overall_confidence_pct:99, linked_instrument_id:"c1000000-0000-0000-0000-000000000001", impact_types:["confirma_cumplimiento"], impact_urgency:"informativa", impact_summary:"ICA Semestral presentado. OBL-04 registrada como cumplida para el periodo.", actions:[{type:"add_evidence",desc:"OBL-04 - Evidencia de radicacion registrada"}] },
  { id:"comm3", original_name:"Oficio_Concepto_Compensacion.pdf", file_type:"pdf", direction:"entrante", comm_type:"concepto", comm_date:"2026-03-15", radicado_number:"2026-018-CRA", authority_name:"AUTORIDAD COMPETENTE - Regional", subject_extracted:"Concepto tecnico sobre areas de compensacion forestal", analysis_status:"requiere_confirmacion", overall_confidence_pct:71, linked_instrument_id:null, impact_types:["modifica_plazo"], impact_urgency:"moderada", impact_summary:"Establece condiciones para la Fase II de compensacion forestal. Requiere confirmacion del EDI.", confirmation_questions:[{question:"Este concepto corresponde al proyecto AS I Baranoa o AS II Polonuevo?",options:["AS I - Baranoa (Res. 786/2016)","AS II - Polonuevo (Res. 556/2017)","Otro"]},{question:"El area N1 mencionada es la de compensacion de OBL-11?",options:["Si, corresponde a OBL-11","No, es otra obligacion","No estoy seguro"]}], actions:[] },
];
...
export default function ComunicacionesModule() {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system:`Eres el motor de analisis de comunicaciones de VIGIA, plataforma de inteligencia regulatoria ambiental colombiana. EDIs disponibles: ${EDIS.map(e=>`${e.name} (Instrumento No. ${e.number})`).join(", ")}. Analiza el nombre del archivo. Responde SOLO en JSON valido: {"comm_type":"auto_seguimiento|requerimiento|notificacion|oficio|respuesta|acta_visita|concepto|sancion|aprobacion|otro","direction":"entrante|saliente","subject_extracted":"asunto","radicado_number":"numero o null","authority_name":"autoridad","candidate_edi":"nombre del EDI o null","candidate_confidence":0-100,"matching_reasons":["razon"],"impact_types":["confirma_cumplimiento|crea_obligacion|modifica_plazo|inicia_sancion|requiere_informacion|aprueba_informe"],"impact_summary":"descripcion","impact_urgency":"critica|moderada|informativa","requires_confirmation":true,"confirmation_questions":[{"question":"...","options":["..."]}]}`,
          messages:[{role:"user",content:`Analiza esta comunicacion ambiental: "${file.name}"`}]
        })
      });
      ...
      setAnalysisResult({ comm_type:"oficio", direction:"entrante", subject_extracted:"Comunicacion ambiental", radicado_number:null, authority_name:"AUTORIDAD COMPETENTE", candidate_edi:EDIS[0].name, candidate_confidence:82, matching_reasons:["Contexto ambiental detectado"], impact_types:["requiere_informacion"], impact_summary:"Documento recibido. Requiere revision.", impact_urgency:"moderada", requires_confirmation:true, confirmation_questions:[{question:"A cual EDI pertenece?",options:EDIS.map(e=>e.name)}], file_name:file.name, file_type:file.name.split(".").pop() });
    }
  ...
  const urgColor = (u) => u==="critica"?C.red:u==="moderada"?C.yellow:C.blue;
  const urgBg = (u) => u==="critica"?C.redDim:u==="moderada"?C.yellowDim:C.blueDim;
  ...
  return (
    <div style={{padding:28,fontFamily:FONT,color:C.text}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <p style={{fontSize:13,color:C.textSec,margin:"4px 0 0"}}>
            {comms.length} documentos {pendingConfirmation>0&&<span style={{color:C.yellow,fontWeight:600}}>- {pendingConfirmation} pendiente(s) de confirmacion</span>}
          </p>
        </div>
        <button onClick={()=>fileRef.current?.click()} style={{background:C.primary,border:"none",borderRadius:8,padding:"9px 16px",color:C.bg,fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:7}}>
          <Upload size={14}/> Subir comunicacion
        </button>
      ...
      {uploadState==="analyzing"&&(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"32px 24px",marginBottom:20,display:"flex",flexDirection:"column",alignItems:"center",gap:16,animation:"fadeIn 0.3s ease"}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {["OCR","Clasificacion","Matching EDI","Impacto"].map((s,i)=>(
      ...
      {uploadState==="result"&&analysisResult&&(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:24,marginBottom:20,animation:"fadeIn 0.3s ease"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
            ...
            <div><div style={{fontSize:14,fontWeight:700,color:C.text}}>Analisis completado</div><div style={{fontSize:11,color:C.textSec}}>{analysisResult.file_name}</div></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            ...
            <div style={{background:C.surfaceEl,borderRadius:10,padding:"12px 14px"}}>
              ...
              {analysisResult.candidate_confidence>=95?<div style={{fontSize:10,color:C.green,marginTop:4}}>Vinculacion automatica</div>:<div style={{fontSize:10,color:C.yellow,marginTop:4}}>Requiere confirmacion</div>}
            </div>
          </div>
          <div style={{background:urgBg(analysisResult.impact_urgency),border:`1px solid ${urgColor(analysisResult.impact_urgency)}44`,borderRadius:10,padding:"12px 16px",marginBottom:16}}>
                ...
                <div key={qi} style={{marginBottom:12}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {q.options.map((opt,oi)=>(
                      <button key={oi} onClick={()=>setConfirmAnswers(p=>({...p,[qi]:opt}))} style={{background:confirmAnswers[qi]===opt?C.primaryDim:C.surface,border:`1px solid ${confirmAnswers[qi]===opt?C.primary:C.border}`,borderRadius:6,padding:"7px 12px",color:confirmAnswers[qi]===opt?C.primary:C.textSec,fontSize:12,cursor:"pointer",textAlign:"left"}}>
                        {opt}
                      </button>
                    ))}
                  </div>
      ...
      {uploadState==="idle"&&(
        <div onClick={()=>fileRef.current?.click()} style={{border:`2px dashed ${C.border}`,borderRadius:12,padding:"28px",textAlign:"center",cursor:"pointer",marginBottom:20}}>
          <Upload size={24} color={C.textMuted} style={{marginBottom:10}}/>
          <div style={{fontSize:14,fontWeight:600,color:C.textSec,marginBottom:4}}>Toca para subir una comunicacion</div>
          <div style={{fontSize:12,color:C.textMuted}}>PDF - DOCX - JPG - PNG - ZIP - MSG - EML</div>
        </div>
      )}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {filteredComms.map(comm=>{
          ...
          const uc=urgColor(comm.impact_urgency);
          const ub=urgBg(comm.impact_urgency);
          return (
            <div key={comm.id} style={{background:C.surface,border:`1px solid ${comm.analysis_status==="requiere_confirmacion"?C.yellow+"55":C.border}`,borderRadius:12,overflow:"hidden"}}>
              <div onClick={()=>setSelectedComm(isSelected?null:comm)} style={{padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                    ...
                    {edi&&<span style={{fontSize:11,color:C.primary}}>EDI: {edi.name}</span>}
                  </div>
                </div>
                <div style={{textAlign:"right",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                  <ConfidenceMeter pct={comm.overall_confidence_pct}/>
                  {comm.impact_urgency&&<Badge label={comm.impact_urgency} color={uc} bg={ub}/>}
                </div>
              ...
              {isSelected&&(
                <div style={{borderTop:`1px solid ${C.border}`,padding:"16px 18px"}}>
                  <div style={{fontSize:12,color:C.textSec,marginBottom:12,lineHeight:1.6,padding:"10px 14px",background:ub,borderRadius:8,borderLeft:`3px solid ${uc}`}}>{comm.impact_summary}</div>
                  {comm.actions?.length>0&&comm.actions.map((a,i)=>(
