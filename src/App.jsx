...
function CommDirectionBadge({direction}) {
  ...
  return <div style={{display:"flex",alignItems:"center",gap:4,color:"#f7c948",fontSize:11,fontWeight:600}}>Bilateral</div>;
}
function CommConfidenceMeter({pct}) {
  const color=pct>=95?"#2ec986":pct>=70?"#f7c948":"#ff4d6d";
  return <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:80,height:4,background:"#101d30",borderRadius:2,overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:2}}/></div><span style={{fontSize:11,fontWeight:700,color}}>{pct}%</span></div>;
}
function IntakeModule() {
  ...
  const analyzeDocument = async (file) => {
    ...
    const canReadContent = isPDF||isImage;
    let base64Data = null;
...
Lee el documento completo y extrae TODA la informacion relevante.
Responde SOLO en JSON valido:
{"doc_nature":"acto_administrativo|comunicacion|evidencia_cumplimiento|documento_tecnico|instrumento_legal|documento_financiero|otro","sender":"nombre completo de quien envia","receiver":"nombre completo de quien recibe","doc_date":"YYYY-MM-DD o null","radicado":"numero exacto o null","subject":"asunto o titulo exacto","content_summary":"resumen preciso 2-3 oraciones","actions_detected":["crea_obligacion|modifica_obligacion|confirma_cumplimiento|inicia_sancion|requiere_respuesta|amplia_plazo|aprueba_tramite|informativo"],"obligations_affected":["OBL-04|OBL-07|OBL-11|OBL-03"],"deadlines_found":["plazos detectados"],"candidate_edi":"nombre exacto del EDI o null","candidate_confidence":0-100,"matching_reasons":["razon basada en contenido real"],"urgency":"critica|moderada|informativa","requires_confirmation":false,"confirmation_questions":[],"recommended_classification":"como clasificar en el EDI"}`;
    try {
      let messageContent;
      if(canReadContent&&base64Data) {
        if(isPDF) {
          messageContent=[{type:"document",source:{type:"base64",media_type:"application/pdf",data:base64Data}},{type:"text",text:`Lee este documento y analiza su contenido para clasificarlo en VIGIA. Nombre: "${file.name}"`}];
        } else {
          messageContent=[{type:"image",source:{type:"base64",media_type:mediaType,data:base64Data}},{type:"text",text:`Lee este documento y analiza su contenido para clasificarlo en VIGIA. Nombre: "${file.name}"`}];
        }
      } else {
        messageContent=`Analiza por nombre e infiere contenido: "${file.name}" (${(file.size/1024/1024).toFixed(1)} MB, tipo: ${file.type||file.name.split(".").pop()})`;
      }
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1500,system:SYSTEM,messages:[{role:"user",content:messageContent}]})});
      const data=await res.json();
      const text=data.content?.[0]?.text||"{}";
      const parsed=JSON.parse(text.replace(/```json|```/g,"").trim());
      setAnalysisResult({...parsed,file_name:file.name,file_type:file.name.split(".").pop(),file_size:`${(file.size/1024/1024).toFixed(1)} MB`,ocr_used:canReadContent&&base64Data!==null});
    } catch {
      setAnalysisResult({doc_nature:"otro",sender:"Por determinar",receiver:"C.I. Energia Solar S.A.S.",doc_date:null,radicado:null,subject:file.name.replace(/\.[^/.]+$/,"").replace(/_/g," "),content_summary:"El motor no pudo procesar el documento. Verifique que no este corrupto o protegido.",actions_detected:["informativo"],obligations_affected:[],deadlines_found:[],candidate_edi:null,candidate_confidence:30,matching_reasons:["Error en procesamiento"],urgency:"informativa",requires_confirmation:true,confirmation_questions:[{question:"Cual es la naturaleza de este documento?",options:Object.values(DOC_TYPES).map(t=>t.label)},{question:"A cual EDI pertenece?",options:[...INTAKE_EDIS.map(e=>e.name),"No corresponde a ningun EDI"]}],recommended_classification:"Clasificar manualmente.",file_name:file.name,file_type:file.name.split(".").pop(),file_size:`${(file.size/1024/1024).toFixed(1)} MB`,ocr_used:false});
    }
  ...
  const processAndLink = () => {
    ...
    setDocs(p=>[{id:`int_${Date.now()}`,original_name:analysisResult.file_name,file_type:analysisResult.file_type,file_size:analysisResult.file_size,doc_nature:analysisResult.doc_nature,sender:analysisResult.sender,receiver:analysisResult.receiver,doc_date:analysisResult.doc_date||new Date().toISOString().split("T")[0],received_date:new Date().toISOString().split("T")[0],radicado:analysisResult.radicado,subject:analysisResult.subject,content_summary:analysisResult.content_summary,actions_detected:analysisResult.actions_detected,obligations_affected:analysisResult.obligations_affected,confidence_pct:analysisResult.candidate_confidence,edi_id:edi?.id||null,urgency:analysisResult.urgency,status:"procesado",processed_date:new Date().toISOString().split("T")[0]},...p]);
  ...
  const filteredDocs=docs.filter(d=>{if(filterNature!=="todos"&&d.doc_nature!==filterNature)return false;if(filterEDI!=="todos"&&d.edi_id!==filterEDI)return false;return true;});
    ...
    <div style={{padding:28,color:I.text}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <p style={{fontSize:13,color:I.textSec,margin:"4px 0 0"}}>Motor de ingestion documental - {docs.length} documento{docs.length!==1?"s":""} procesado{docs.length!==1?"s":""}
      ...
      {uploadState==="idle"&&(
        <div onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={e=>{e.preventDefault();setDragOver(false);e.dataTransfer.files?.[0]&&analyzeDocument(e.dataTransfer.files[0]);}} onClick={()=>fileRef.current?.click()}
          ...
          <div style={{fontSize:12,color:I.textMuted,marginBottom:16}}>PDF - DOCX - XLSX - JPG - PNG - ZIP - MSG - EML - Cualquier formato</div>
      ...
      {uploadState==="result"&&analysisResult&&(
        <div style={{background:I.surface,border:`1px solid ${I.border}`,borderRadius:14,padding:24,marginBottom:24,animation:"fadeInI 0.3s ease"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
            ...
            <div><div style={{fontSize:15,fontWeight:700,color:I.text}}>Analisis completado</div><div style={{fontSize:11,color:I.textSec}}>{analysisResult.file_name} - {analysisResult.file_size}</div></div>
          ...
          {analysisResult.requires_confirmation&&analysisResult.confirmation_questions?.length>0&&(
            <div style={{background:"rgba(247,201,72,0.06)",border:`1px solid ${I.yellow}33`,borderRadius:10,padding:"14px 16px",marginBottom:16}}>
              <div style={{fontSize:12,fontWeight:700,color:I.yellow,marginBottom:12,display:"flex",alignItems:"center",gap:6}}><Eye size={13}/>Confianza menor al 95% - confirma para clasificar correctamente</div>
          ...
          <div style={{padding:"10px 14px",background:urgB(analysisResult.urgency),borderRadius:8,marginBottom:16,fontSize:12,color:urgC(analysisResult.urgency),fontWeight:500,borderLeft:`3px solid ${urgC(analysisResult.urgency)}`}}>{analysisResult.recommended_classification}</div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {filteredDocs.map(doc=>{
          ...
          return (
            <div key={doc.id} style={{background:I.surface,border:`1px solid ${doc.status==="requiere_confirmacion"?I.yellow+"55":I.border}`,borderRadius:12,overflow:"hidden"}} onMouseEnter={e=>e.currentTarget.style.borderColor=nt.color+"44"} onMouseLeave={e=>e.currentTarget.style.borderColor=doc.status==="requiere_confirmacion"?I.yellow+"55":I.border}>
              <div onClick={()=>setSelectedDoc(isSelected?null:doc)} style={{padding:"14px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:42,height:42,borderRadius:10,background:nt.bg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><FileText size={18} color={nt.color}/></div>
...
export default function VIGIAApp() {
  ...
  const renderDashboard=()=>(
    <div style={{padding:28}}>
      <div style={{marginBottom:24}}><h1 style={{fontSize:22,fontWeight:700,color:C.text,margin:0}}>Panel de cumplimiento</h1><p style={{fontSize:13,color:C.textSec,margin:"4px 0 0"}}>{dbStatus==="connected"?`Sincronizado con Supabase · ${lastSync?.toLocaleTimeString("es-CO")}`:"Modo demo · Datos reales de C.I. Energía Solar — Proyecto AS I Baranoa"}</p></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 360px",gap:16}}>
        <div>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}><span style={{fontSize:13,fontWeight:600,color:C.text}}>Expedientes Digitales Inteligentes</span><span style={{fontSize:11,color:C.textSec}}>{instruments.length} EDI{instruments.length!==1?"s":""} activo{instruments.length!==1?"s":""}</span></div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {instruments.map(inst=>{ const health=ediHealth(inst); const obs=ediObs(inst.id); const color=health==="crítico"?C.red:health==="moderado"?C.yellow:C.green; const bg=health==="crítico"?C.redDim:health==="moderado"?C.yellowDim:C.greenDim; return <div key={inst.id} onClick={()=>{setSelectedEDI(inst);setView("edi-detail");}} style={{background:C.surface,border:`1px solid ${health==="crítico"?C.red+"44":C.border}`,borderRadius:12,padding:"16px 18px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor=color+"88"} onMouseLeave={e=>e.currentTarget.style.borderColor=health==="crítico"?C.red+"44":C.border}><div style={{display:"flex",alignItems:"center",gap:14}}><div style={{width:42,height:42,borderRadius:10,background:bg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><FileText size={18} color={color}/></div><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}><span style={{fontSize:14,fontWeight:600,color:C.text}}>{inst.projects?.name}</span><StatusDot status={health}/></div><div style={{fontSize:11,color:C.textSec,marginBottom:6}}>Instrumento N.º {inst.number} · AUTORIDAD COMPETENTE — Nivel {inst.authority_level}</div><div style={{display:"flex",gap:12}}>{obs.filter(o=>o.status==="vencido").length>0&&<span style={{fontSize:11,color:C.red}}>● {obs.filter(o=>o.status==="vencido").length} vencida(s)</span>}{obs.filter(o=>o.status==="próximo").length>0&&<span style={{fontSize:11,color:C.yellow}}>● {obs.filter(o=>o.status==="próximo").length} próxima(s)</span>}<span style={{fontSize:11,color:C.green}}>● {obs.filter(o=>o.status==="al_día").length} al día</span></div></div><div style={{textAlign:"right",flexShrink:0}}><div style={{marginBottom:4}}><Badge label={`${inst.completeness_pct}% completo`} color={inst.completeness_pct<80?C.yellow:C.green} bg={inst.completeness_pct<80?C.yellowDim:C.greenDim}/></div><ChevronRight size={14} color={C.textSec}/></div></div></div>; })}
          </div>
        </div>
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {alerts.map(alert=>{ const color=alert.urgency==="crítica"?C.red:alert.urgency==="moderada"?C.yellow:C.blue; const bg=alert.urgency==="crítica"?C.redDim:alert.urgency==="moderada"?C.yellowDim:C.blueDim; return <div key={alert.id} onClick={()=>setView("inteligencia")} style={{background:C.surface,border:`1px solid ${!alert.human_validated?color+"55":C.border}`,borderRadius:10,padding:"14px 16px",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.borderColor=color+"88"} onMouseLeave={e=>e.currentTarget.style.borderColor=!alert.human_validated?color+"55":C.border}><div style={{display:"flex",alignItems:"flex-start",gap:10}}><div style={{width:28,height:28,borderRadius:8,background:bg,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{alert.urgency==="crítica"?<AlertTriangle size={13} color={color}/>:alert.urgency==="moderada"?<Clock size={13} color={color}/>:<Eye size={13} color={color}/>}</div><div style={{flex:1}}>{!alert.human_validated&&<div style={{width:6,height:6,borderRadius:"50%",background:color,marginBottom:4,display:"inline-block",marginRight:4}}/>}<div style={{fontSize:12,fontWeight:600,color:C.text,lineHeight:1.4,marginBottom:6}}>{alert.norm_title}</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><ImpactBadge impact={alert.impact_type}/><span style={{fontSize:10,color:C.textMuted}}>{alert.norm_date}</span></div></div></div></div>; })}
          </div>
        </div>
      </div>
    </div>
  ...
  const renderEDIDetail=()=>{ const inst=selectedEDI; if(!inst)return null; const obs=ediObs(inst.id); const health=ediHealth(inst); const sc=health==="crítico"?C.red:health==="moderado"?C.yellow:C.green; return <div style={{padding:28}}><button onClick={()=>setView("dashboard")} style={{background:"transparent",border:"none",color:C.textSec,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",gap:5,marginBottom:20,padding:0}}>← Volver al panel</button><div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 24px",marginBottom:20}}><div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:12}}><div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}><h2 style={{fontSize:20,fontWeight:700,color:C.text,margin:0}}>{inst.projects?.name}</h2><StatusDot status={health} size={10}/></div><div style={{fontSize:12,color:C.textSec,marginBottom:10}}>Instrumento Ambiental N.º {inst.number} · AUTORIDAD COMPETENTE — Nivel {inst.authority_level}</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Badge label={inst.domain} color={C.primary} bg={C.primaryDim}/><Badge label={inst.instrument_type?.replace(/_/g," ")} color={C.textSec} bg={C.surfaceEl}/><Badge label={`${inst.completeness_pct}% completitud`} color={inst.completeness_pct<80?C.yellow:C.green} bg={inst.completeness_pct<80?C.yellowDim:C.greenDim}/>{inst.completeness_pct<100&&<Badge label="⚠ Expediente incompleto" color={C.yellow} bg={C.yellowDim}/>}</div></div><button onClick={()=>setView("consultar")} style={{background:C.primaryDim,border:`1px solid ${C.primary}44`,color:C.primary,borderRadius:8,padding:"8px 14px",fontSize:12,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6,flexShrink:0}}><MessageSquare size={13}/>Consultar sobre este EDI</button></div><div style={{marginTop:16}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,color:C.textSec}}>Cumplimiento general</span><span style={{fontSize:11,fontWeight:600,color:sc}}>{obs.length>0?Math.round((obs.filter(o=>o.status==="al_día").length/obs.length)*100):0}%</span></div><div style={{background:C.surfaceEl,borderRadius:4,height:8,overflow:"hidden"}}><div style={{width:`${obs.length>0?(obs.filter(o=>o.status==="al_día").length/obs.length)*100:0}%`,height:"100%",background:`linear-gradient(90deg,${sc},${sc}88)`,borderRadius:4}}/></div><div style={{display:"flex",gap:16,marginTop:8}}><span style={{fontSize:10,color:C.red}}>● {obs.filter(o=>o.status==="vencido").length} vencida(s)</span><span style={{fontSize:10,color:C.yellow}}>● {obs.filter(o=>o.status==="próximo").length} próxima(s)</span><span style={{fontSize:10,color:C.green}}>● {obs.filter(o=>o.status==="al_día").length} al día</span></div></div></div><div style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:12}}>Obligaciones del expediente</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{obs.map(ob=>{ const color=ob.status==="vencido"?C.red:ob.status==="próximo"?C.yellow:C.green; const bg=ob.status==="vencido"?C.redDim:ob.status==="próximo"?C.yellowDim:C.greenDim; const days=ob.due_date?Math.ceil((new Date(ob.due_date)-new Date())/86400000):null; return <div key={ob.id} style={{background:C.surface,border:`1px solid ${ob.status==="vencido"?C.red+"55":C.border}`,borderRadius:10,padding:"14px 18px",display:"grid",gridTemplateColumns:"auto 1fr auto",alignItems:"center",gap:14}}><div style={{width:38,height:38,borderRadius:8,background:bg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{ob.status==="vencido"?<AlertTriangle size={16} color={color}/>:ob.status==="próximo"?<Clock size={16} color={color}/>:<CheckCircle size={16} color={color}/>}</div><div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:13,fontWeight:600,color:C.text}}>{ob.name}</span><span style={{fontSize:10,color:C.textMuted,fontFamily:"monospace"}}>{ob.obligation_num}</span></div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><Badge label={ob.obligation_type?.replace(/_/g," ")} color={C.textSec} bg={C.surfaceEl}/><Badge label={ob.frequency} color={C.blue} bg={C.blueDim}/><span style={{fontSize:10,color:C.textMuted}}>{ob.source_article}</span><Badge label={`${ob.confidence_level} confianza`} color={ob.confidence_level==="alta"?C.green:ob.confidence_level==="media"?C.yellow:C.red} bg={ob.confidence_level==="alta"?C.greenDim:ob.confidence_level==="media"?C.yellowDim:C.redDim}/></div></div><div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:20,fontWeight:700,color,lineHeight:1}}>{days!==null?Math.abs(days):"—"}</div><div style={{fontSize:10,color:C.textMuted}}>{days!==null?(days<0?"días vencido":"días restantes"):""}</div><div style={{fontSize:11,color:C.textSec,marginTop:2}}>{ob.due_date}</div></div></div>; })}</div></div>; };

  const renderInteligencia=()=><div style={{padding:28}}><h1 style={{fontSize:22,fontWeight:700,color:C.text,margin:"0 0 6px"}}>Inteligencia regulatoria</h1><p style={{fontSize:13,color:C.textSec,margin:"0 0 24px"}}>Motor de monitoreo continuo · {alerts.length} alertas activas</p><div style={{display:"flex",flexDirection:"column",gap:14}}>{alerts.map(alert=>{ const color=alert.urgency==="crítica"?C.red:alert.urgency==="moderada"?C.yellow:C.blue; const bg=alert.urgency==="crítica"?C.redDim:alert.urgency==="moderada"?C.yellowDim:C.blueDim; return <div key={alert.id} style={{background:C.surface,border:`1px solid ${!alert.human_validated?color+"55":C.border}`,borderRadius:14,padding:"20px 22px"}}><div style={{display:"flex",alignItems:"flex-start",gap:16}}><div style={{width:44,height:44,borderRadius:12,background:bg,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{alert.urgency==="crítica"?<AlertTriangle size={20} color={color}/>:alert.urgency==="moderada"?<Zap size={20} color={color}/>:<Eye size={20} color={color}/>}</div><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8,flexWrap:"wrap"}}><ImpactBadge impact={alert.impact_type}/>{!alert.human_validated&&<Badge label="Nueva" color={color} bg={bg}/>}<span style={{fontSize:11,color:C.textMuted}}>{alert.norm_date}</span></div><div style={{fontSize:15,fontWeight:700,color:C.text,marginBottom:4,lineHeight:1.4}}>{alert.norm_title}</div><div style={{fontSize:12,color:C.textSec,marginBottom:12}}>{alert.issuing_authority}</div><div style={{fontSize:13,color:C.text,marginBottom:10,lineHeight:1.7,padding:"12px 16px",background:C.surfaceEl,borderRadius:8,borderLeft:`3px solid ${color}`}}>{alert.summary}</div>{alert.detailed_analysis&&<div style={{fontSize:12,color:C.textSec,marginBottom:12,lineHeight:1.6}}>{alert.detailed_analysis}</div>}<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}><div style={{fontSize:11,color:C.textMuted}}>Confianza: <span style={{color:(alert.confidence_pct||0)>85?C.green:C.yellow,fontWeight:700}}>{alert.confidence_pct}%</span></div><button onClick={()=>setAlerts(p=>p.map(a=>a.id===alert.id?{...a,human_validated:true}:a))} style={{background:C.surfaceEl,border:`1px solid ${C.border}`,borderRadius:6,padding:"4px 12px",fontSize:11,color:C.textSec,cursor:"pointer"}}>Marcar revisado</button></div>{alert.suggested_action&&<div style={{marginTop:12,padding:"10px 16px",background:bg,borderRadius:8,fontSize:12,color,fontWeight:600}}>→ {alert.suggested_action}</div>}</div></div></div>; })}</div></div>;

  const renderOversight=()=><div style={{padding:28}}><h1 style={{fontSize:22,fontWeight:700,color:C.text,margin:"0 0 6px"}}>Oversight legal automático</h1><p style={{fontSize:13,color:C.textSec,margin:"0 0 24px"}}>Anomalías detectadas · {oversight.length} activas</p><div style={{display:"flex",flexDirection:"column",gap:12}}>{oversight.map(ov=>{ const color=ov.severity==="crítico"?C.red:ov.severity==="moderado"?C.yellow:C.blue; const bg=ov.severity==="crítico"?C.redDim:ov.severity==="moderado"?C.yellowDim:C.blueDim; return <div key={ov.id} style={{background:C.surface,border:`1px solid ${color+"55"}`,borderRadius:12,padding:"18px 22px"}}><div style={{display:"flex",alignItems:"flex-start",gap:14}}><div style={{width:40,height:40,borderRadius:10,background:bg,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><AlertTriangle size={18} color={color}/></div><div style={{flex:1}}><div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}><Badge label={ov.severity} color={color} bg={bg}/><Badge label={ov.anomaly_type?.replace(/_/g," ")} color={C.textSec} bg={C.surfaceEl}/></div><div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:6}}>{ov.title}</div><div style={{fontSize:13,color:C.textSec,marginBottom:10,lineHeight:1.6}}>{ov.description}</div><div style={{fontSize:11,color:C.textMuted,marginBottom:4}}>Referencia legal: <span style={{color:C.text}}>{ov.legal_reference}</span></div><div style={{marginTop:12,padding:"10px 14px",background:bg,borderRadius:8,fontSize:12,color,fontWeight:600}}>→ {ov.suggested_action}</div></div></div></div>; })}</div></div>;

  const renderNormativa=()=><div style={{padding:28}}><h1 style={{fontSize:22,fontWeight:700,color:C.text,margin:"0 0 6px"}}>Base normativa — Capa 2</h1><p style={{fontSize:13,color:C.textSec,margin:"0 0 24px"}}>{normSources.length} normas activas · Colombia · Actualización continua</p><div style={{display:"flex",flexDirection:"column",gap:10}}>{normSources.map(n=><div key={n.id} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 18px",display:"flex",alignItems:"center",gap:14}}><div style={{width:36,height:36,borderRadius:8,background:C.primaryDim,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><BookOpen size={15} color={C.primary}/></div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.text,marginBottom:4}}>{n.norm_title}</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Badge label={n.norm_type} color={C.blue} bg={C.blueDim}/>{n.norm_number&&<Badge label={`N.º ${n.norm_number}`} color={C.textSec} bg={C.surfaceEl}/>}<span style={{fontSize:10,color:C.textMuted}}>{n.issue_date}</span></div></div><Badge label="Vigente" color={C.green} bg={C.greenDim}/></div>)}</div></div>;

  const renderConsultar=()=>{ const c=conf(); return <div style={{height:"100%",display:"flex",flexDirection:"column",padding:28,gap:16}}><div><h1 style={{fontSize:22,fontWeight:700,color:C.text,margin:0}}>Motor de consulta</h1><p style={{fontSize:13,color:C.textSec,margin:"4px 0 0"}}>{normSources.length} normas · {obligations.length} obligaciones · respuestas con trazabilidad jurídica</p></div><div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 18px"}}><div style={{fontSize:11,fontWeight:700,color:C.textSec,marginBottom:12,textTransform:"uppercase",letterSpacing:"0.1em"}}>Fuentes de consulta</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>{[{key:"documentos",icon:Database,label:"Mis documentos",sub:"Capa 1 — EDIs propios",color:C.primary},{key:"normativa",icon:BookOpen,label:"Normativa vigente",sub:`Capa 2 · ${normSources.length} normas`,color:C.blue},{key:"jurisprudencia",icon:Shield,label:"Jurisprudencia",sub:"Capa 2 — Tribunales y cortes",color:C.purple},{key:"validacion",icon:Eye,label:"Validación humana",sub:"Capa 3 — ENARA / Firma",color:C.yellow}].map(({key,icon:Icon,label,sub,color})=>(<div key={key} onClick={()=>toggleSource(key)} style={{background:sources[key]?`${color}12`:C.surfaceEl,border:`1px solid ${sources[key]?color+"66":C.border}`,borderRadius:8,padding:"10px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}><div style={{width:28,height:28,borderRadius:6,background:sources[key]?`${color}22`:C.border+"44",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={13} color={sources[key]?color:C.textMuted}/></div><div><div style={{fontSize:12,fontWeight:600,color:sources[key]?C.text:C.textSec}}>{label}</div><div style={{fontSize:10,color:C.textMuted}}>{sub}</div></div></div>))}</div><div style={{padding:"8px 12px",borderRadius:8,background:c.color===C.green?C.greenDim:c.color===C.red?C.redDim:C.yellowDim,fontSize:12,color:c.color,fontWeight:500}}>{c.risk} {c.label}</div></div><div style={{flex:1,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0}}><div style={{flex:1,overflowY:"auto",padding:"16px 18px",display:"flex",flexDirection:"column",gap:12}}>{botMessages.map((msg,i)=>(<div key={i} style={{display:"flex",flexDirection:msg.role==="user"?"row-reverse":"row",alignItems:"flex-start",gap:10}}>{msg.role!=="user"&&<div style={{width:28,height:28,borderRadius:8,background:C.primaryDim,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Zap size={13} color={C.primary}/></div>}<div style={{maxWidth:"78%",background:msg.role==="user"?C.primaryDim:C.surfaceEl,border:`1px solid ${msg.role==="user"?C.primary+"44":C.border}`,borderRadius:msg.role==="user"?"12px 4px 12px 12px":"4px 12px 12px 12px",padding:"10px 14px"}}>{msg.role==="system"&&<div style={{fontSize:10,color:C.primary,fontWeight:700,marginBottom:6,textTransform:"uppercase"}}>VIGÍA</div>}<div style={{fontSize:13,color:C.text,lineHeight:1.65,whiteSpace:"pre-wrap"}}>{msg.text}</div>{msg.layers&&msg.role==="assistant"&&<div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.border}`,fontSize:10,color:C.textMuted}}>Fuentes: {msg.layers}</div>}</div></div>))}{botLoading&&<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:28,height:28,borderRadius:8,background:C.primaryDim,display:"flex",alignItems:"center",justifyContent:"center"}}><Zap size={13} color={C.primary}/></div><div style={{background:C.surfaceEl,border:`1px solid ${C.border}`,borderRadius:"4px 12px 12px 12px",padding:"12px 16px",display:"flex",gap:5}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:C.primary,animation:"pulse 1.2s infinite",animationDelay:`${i*0.25}s`}}/>)}</div></div>}</div><div style={{padding:"12px 16px",borderTop:`1px solid ${C.border}`,display:"flex",gap:10,alignItems:"flex-end"}}><textarea value={botInput} onChange={e=>setBotInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendBot();}}} placeholder="Escribe tu consulta... (Enter para enviar)" style={{flex:1,background:C.surfaceEl,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 14px",color:C.text,fontSize:13,fontFamily:FONT,resize:"none",minHeight:42,maxHeight:120,outline:"none",lineHeight:1.5}} rows={1}/><button onClick={sendBot} disabled={botLoading||!botInput.trim()} style={{background:botLoading||!botInput.trim()?C.surfaceEl:C.primary,border:"none",borderRadius:8,padding:"11px 16px",cursor:"pointer",flexShrink:0}}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={botLoading||!botInput.trim()?C.textMuted:"#060c14"} strokeWidth="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button></div></div></div>; };
