...
const renderView=()=>{ if(view==="intake")return <IntakeModule onNewAlert={handleNewAlert} onNewNorm={handleNewNorm}/>; if(view==="superadmin")return <SuperAdminModule/>; if(view==="edi-detail")return renderEDIDetail(); if(view==="inteligencia")return renderInteligencia(); if(view==="consultar")return renderConsultar(); if(view==="normativa")return renderNormativa(); if(view==="oversight")return renderOversight(); return renderDashboard(); };
...
<div><div style={{fontSize:16,fontWeight:800,color:C.text,letterSpacing:"-0.03em"}}>VIGIA</div><div style={{fontSize:9,color:C.textSec,textTransform:"uppercase",letterSpacing:"0.12em",marginTop:1}}>Inteligencia Regulatoria</div></div>
</div>
</div>
...
function SuperAdminModule() {
  ...
  const createUser = async () => {
    if(!newUser.email || !newUser.password) { setMessage({type:"error",text:"Email y password requeridos"}); return; }
    setLoading(true); setMessage(null);
    try {
      // 1. Create auth user
      const userRes = await adminFetch("/auth/v1/admin/users", "POST", {
        email: newUser.email,
      ...
      // 3. Link user to org
      if(orgId) {
        await adminFetch("/rest/v1/user_org_map", "POST", {
          user_id: userRes.id,
          org_id: orgId,
          role: newUser.role
        });
  ...
  const deleteUser = async (userId, email) => {
    ...
    setLoading(true);
    await adminFetch(`/auth/v1/admin/users/${userId}`, "DELETE");
    setMessage({type:"success",text:`Usuario ${email} eliminado`});
    loadData();
  };
  ...
  const runSetup = async () => {
    ...
    try {
      ...
      for(const u of users) {
        ...
        const ur = await sPost("/auth/v1/admin/users",{email:u.email,password:u.password,email_confirm:true});
        if(!ur.id){ addLog("ERROR "+u.email+": "+(ur.message||JSON.stringify(ur).slice(0,60)),"error"); continue; }
        await sPost("/rest/v1/user_org_map",{user_id:ur.id,org_id:u.org_id,role:u.role},"resolution=merge-duplicates,return=minimal");
        addLog(u.email+" ("+u.role+") OK","success");
      }
      addLog("SETUP COMPLETO - Recarga SuperAdmin para ver los cambios","success");
      setSetupDone(true);
      loadData();
  ...
  return (
    <div style={{padding:28,color:A.text}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        <div style={{width:40,height:40,borderRadius:10,background:"rgba(255,77,109,0.15)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <Shield size={20} color={A.red}/>
        </div>
        <div>
          <h1 style={{fontSize:22,fontWeight:700,color:A.text,margin:0}}>SUPERADMIN</h1>
          <p style={{fontSize:12,color:A.red,margin:0,fontWeight:600}}>ACCESO RESTRINGIDO - ENARA CONSULTING</p>
        </div>
        <button onClick={loadData} style={{marginLeft:"auto",background:A.surfaceEl,border:`1px solid ${A.border}`,borderRadius:6,padding:"6px 12px",color:A.textSec,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
          <RefreshCw size={12}/> Actualizar
        </button>
      </div>

      {message&&(
        <div style={{background:message.type==="success"?A.greenDim:A.redDim,border:`1px solid ${message.type==="success"?A.green:A.red}44`,borderRadius:8,padding:"10px 14px",fontSize:12,color:message.type==="success"?A.green:A.red,marginBottom:16}}>
          {message.text}
        </div>
      )}
      ...
      <div style={{display:"flex",gap:6,marginBottom:20}}>
        {tabs.map(t=>(
          <button key={t.k} onClick={()=>setTab(t.k)} style={{background:tab===t.k?A.primaryDim:A.surface,border:`1px solid ${tab===t.k?A.primary+"44":A.border}`,borderRadius:6,padding:"6px 14px",color:tab===t.k?A.primary:A.textSec,fontSize:12,fontWeight:tab===t.k?600:400,cursor:"pointer"}}>{t.l}</button>
        ))}
      </div>
      ...
      {tab==="overview"&&(
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
            {[["Usuarios",stats.users,A.primary],["Organizaciones",stats.orgs,A.blue],["Obligaciones",stats.obligations,A.yellow],["Alertas",stats.alerts,A.red]].map(([l,v,c])=>(
              <div key={l} style={{background:A.surface,border:`1px solid ${A.border}`,borderRadius:12,padding:"16px 20px"}}>
                <div style={{fontSize:28,fontWeight:700,color:c}}>{v}</div>
                <div style={{fontSize:12,color:A.textSec,marginTop:4}}>{l}</div>
              </div>
            ))}
          </div>
          <div style={{background:A.surface,border:`1px solid ${A.border}`,borderRadius:12,padding:"16px 20px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[["Supabase","itkbujkqjesuntgdkubt (Sao Paulo)"],["GitHub","Yopakhome/Vigia"],["Vercel","vigia-five.vercel.app"],["Modelo IA","claude-sonnet-4-20250514"],["Stack","React + Vite + Supabase"],["Auth","Supabase Auth + RLS"]].map(([k,v])=>(
                <div key={k} style={{background:A.surfaceEl,borderRadius:8,padding:"10px 12px"}}>
                  <div style={{fontSize:10,color:A.textMuted,marginBottom:2,textTransform:"uppercase"}}>{k}</div>
                  <div style={{fontSize:11,color:A.text,fontWeight:500}}>{v}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab==="users"&&(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {loading&&<div style={{color:A.textSec,fontSize:13}}>Cargando usuarios...</div>}
          {users.map(u=>(
            <div key={u.id} style={{background:A.surface,border:`1px solid ${A.border}`,borderRadius:10,padding:"14px 18px",display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:36,height:36,borderRadius:8,background:A.primaryDim,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                <span style={{fontSize:14,fontWeight:700,color:A.primary}}>{u.email?.[0]?.toUpperCase()}</span>
              </div>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:A.text}}>{u.email}</div>
                <div style={{display:"flex",gap:8,marginTop:3}}>
                  <span style={{fontSize:10,color:A.textMuted,fontFamily:"monospace"}}>{u.id?.slice(0,8)}...</span>
                  <span style={{fontSize:10,color:u.email_confirmed_at?A.green:A.yellow}}>{u.email_confirmed_at?"Verificado":"Pendiente"}</span>
                  <span style={{fontSize:10,color:A.textMuted}}>{u.last_sign_in_at?new Date(u.last_sign_in_at).toLocaleDateString("es-CO"):"Nunca"}</span>
                </div>
              </div>
              <button onClick={()=>deleteUser(u.id,u.email)} style={{background:"transparent",border:`1px solid ${A.red}44`,borderRadius:6,padding:"4px 10px",color:A.red,fontSize:11,cursor:"pointer"}}>Eliminar</button>
            </div>
          ))}
        </div>
      )}
      ...
      {tab==="orgs"&&(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {orgs.map(o=>(
            <div key={o.id} style={{background:A.surface,border:`1px solid ${A.border}`,borderRadius:10,padding:"14px 18px"}}>
      ...
      {tab==="setup"&&(
        <div style={{maxWidth:520}}>
          <div style={{background:A.surface,border:"1px solid "+A.border,borderRadius:14,padding:24,marginBottom:16}}>
            ...
            <div style={{fontSize:12,color:A.textSec,lineHeight:1.6,marginBottom:16}}>Crea las 2 organizaciones demo y los 5 usuarios con un clic. Password para todos: Vigia2026!</div>
            <div style={{background:A.surfaceEl,borderRadius:8,padding:"10px 14px",marginBottom:16}}>
              {[["admin@enara.co","admin","C.I. Energia Solar"],["consulta1@demo-energia.co","viewer","Energia Demo"],["onboarding1@demo-energia.co","editor","Energia Demo"],["consulta2@demo-mineria.co","viewer","Mineria Demo"],["onboarding2@demo-mineria.co","editor","Mineria Demo"]].map(function(item){
                return <div key={item[0]} style={{display:"flex",gap:8,marginBottom:5,alignItems:"center",fontSize:11}}>
                  <span style={{padding:"1px 6px",borderRadius:3,background:item[1]==="editor"?A.primaryDim:item[1]==="admin"?A.redDim:A.surfaceEl,color:item[1]==="editor"?A.primary:item[1]==="admin"?A.red:A.textSec,fontWeight:600,fontSize:10}}>{item[1]}</span>
                  <span style={{color:A.text}}>{item[0]}</span>
                  <span style={{color:A.textMuted}}>- {item[2]}</span>
                </div>;
              })}
            </div>
            <button onClick={runSetup} disabled={setupRunning||setupDone} style={{width:"100%",background:setupDone?A.greenDim:setupRunning?A.surfaceEl:A.red,border:"1px solid "+(setupDone?A.green:A.red)+"44",borderRadius:8,padding:"12px",color:setupDone?A.green:setupRunning?A.textSec:"#fff",fontSize:14,fontWeight:700,cursor:setupRunning||setupDone?"not-allowed":"pointer"}}>
              {setupDone?"Setup completado":setupRunning?"Ejecutando...":"Ejecutar setup demo"}
            </button>
          </div>
          {setupLog.length>0&&(
            <div style={{background:A.surface,border:"1px solid "+A.border,borderRadius:10,padding:"12px 16px",maxHeight:280,overflowY:"auto"}}>
              {setupLog.map(function(l,i){ return <div key={i} style={{fontSize:11,color:l.type==="success"?A.green:l.type==="error"?A.red:A.textSec,marginBottom:3,display:"flex",gap:8}}><span style={{color:A.textMuted,flexShrink:0}}>{l.t}</span><span>{l.msg}</span></div>; })}
            </div>
          )}
        </div>
      )}
          ...
          <div style={{marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:8}}>
              {["admin","editor","viewer"].map(r=>(
                <button key={r} onClick={()=>setNewUser(p=>({...p,role:r}))} style={{background:newUser.role===r?A.primaryDim:A.surfaceEl,border:`1px solid ${newUser.role===r?A.primary+"44":A.border}`,borderRadius:6,padding:"6px 16px",color:newUser.role===r?A.primary:A.textSec,fontSize:12,cursor:"pointer",textTransform:"capitalize"}}>{r}</button>
              ))}
            </div>
          </div>
          <button onClick={createUser} disabled={loading} style={{width:"100%",background:loading?A.surfaceEl:A.primary,border:"none",borderRadius:8,padding:"12px",color:loading?"#5e7a95":"#060c14",fontSize:14,fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:FONT}}>
