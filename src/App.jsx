...
const EXPORT_VIGIA_VERSION = "v3.15.2";
...
const loadRadarApplicable=async()=>{
  ...
  try{
    const r=await fetch(`${SB_URL}/rest/v1/norma_applicability?applies=eq.true&select=*,detected_items(title,excerpt,external_url,resolved_official_url,url_resolution_source,source_key,classification,raw_payload)&order=urgency.desc,matched_at.desc&limit=100`,{headers:{apikey:SB_KEY,Authorization:`Bearer ${session.access_token}`}});
    const d=await r.json();
    ...
    {filtered.map(item=>{const d=item.detected_items||{};const isNew=item.client_status==="nuevo";const cl=d.classification||{};
      return <div key={item.id} onClick={()=>{if(isNew)markRadarItemAction(item.id,"visto");}} style={{background:C.surface,border:`1px solid ${C.border}`,borderLeft:`4px solid ${urgencyColor(item.urgency)}`,borderRadius:10,padding:16,marginBottom:10,position:"relative"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginTop:12}}>
          {(()=>{const url=d.resolved_official_url||d.external_url;const src=d.url_resolution_source;const srcLabel={secretariasenado:"Secretaría del Senado",google_suin:"Buscar en SUIN",eva_directo:"Gestor Normativo EVA",suin_directo:"SUIN-Juriscol",fallback:"Buscador oficial"}[src]||"Fuente oficial";return url?<a href={url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} title={`Fuente: ${srcLabel}`} style={{background:C.primary,color:"#060c14",padding:"6px 14px",borderRadius:6,fontSize:11,fontWeight:700,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:6}}>Ver norma oficial <span style={{fontSize:9,opacity:0.7,fontWeight:500}}>({srcLabel})</span></a>:<button onClick={e=>{e.stopPropagation();const rp=d.raw_payload||{};const ref=(rp.tipo&&rp.n_mero&&rp.a_o)?`${rp.tipo} ${rp.n_mero} de ${rp.a_o}`:(d.title||"").slice(0,80);navigator.clipboard.writeText(ref);}} style={{background:"transparent",border:`1px solid ${C.border}`,color:C.textSec,padding:"6px 12px",borderRadius:6,fontSize:11,fontWeight:500,cursor:"pointer",fontFamily:FONT}}>Copiar referencia (URL pendiente)</button>;})()}
...
<div style={{flex:1}}><div style={{fontSize:16,fontWeight:800,color:C.text,letterSpacing:"-0.03em"}}>VIGIA</div><div style={{fontSize:9,color:C.textSec,textTransform:"uppercase",letterSpacing:"0.12em",marginTop:1}}>Inteligencia Regulatoria</div><div style={{fontSize:9,color:C.primary,fontWeight:700,marginTop:2}}>v3.15.2</div></div>
