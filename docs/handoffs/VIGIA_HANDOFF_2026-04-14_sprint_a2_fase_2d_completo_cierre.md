# VIGÍA — Handoff Brief (cierre Sprint A2 Fases 2A/2B/2D completo + v3.9.1 ENARA REGLA 13)

Este archivo: `VIGIA_HANDOFF_2026-04-14_sprint_a2_fase_2d_completo_cierre.md` | También disponible como: `VIGIA_HANDOFF_LATEST.md`
...
| Campo | Valor |
|---|---|
| Generado | 2026-04-13 cierre + 2026-04-13/14 múltiples updates Sprint A2 + **2026-04-14 cierre completo Fase 2D + v3.9.1 ENARA REGLA 13 + chat-bot v7 deployed** (`America/Bogota`) |
| Fase actual | **Sprint A2 Fases 1A, 1B, 2A, 2B, 2D-suin, 2D-inline-anla, 2D-minambiente-pdf, 2D-minambiente-html, 2D-sisjur, 2D-funcionpublica, 2D-outliers TODAS CERRADAS**. Corpus EUREKA en Supabase: 288 normas + 120 sentencias + **11.661 artículos embedidos** + 390 resúmenes curados + 2.928 concordancias. 7 docs registrados en `corpus_gaps.json` como pendientes (5 OCR + 1 sin URL + 1 SSL obsoleto). |
| Versión del producto | **v3.9.1** — incluye REGLA 13 ENARA Consulting deployed en chat-bot edge function (v7 ACTIVE en Supabase) + bump string en App.jsx |
| Último commit pusheado a origin/main | (este handoff update) — el commit anterior es `<TBD>` con outliers + funcionpublica + corpus_gaps.json |
...
| Commits del día 2026-04-14 | `247ed7c` Fase 1B Jurisprudencia recon → `df402b3` handoff Fase 1B → `2cd085d` v3.8.0 seed demo (push externo) → `476310c` Fase 2A ingesta Normativa → `863d711` handoff Fase 2A → `e06985d` Fase 2B ingesta Jurisprudencia → `e5aca67` v3.9.0 Fase 2D-suin scraper SUIN → `e82f462` handoff Fase 2D-suin → `60de4c8` v3.9.0 bump version → `50d6d43` Fase 2D-inline-anla → `03b3e0a` v3.9.1 chat-bot REGLA 13 ENARA → `9f6f674` Fase 2D-minambiente-pdf → `8650ba4` Fase 2D-minambiente-html + sisjur → `abaf2a2` corpus_gaps.json → (último cierre 2D + handoff) |
| Edge function deploy | **chat-bot v7 deployed via MCP** (id `f9fa02a6`, REGLA 13 ENARA Consulting LIVE en producción) |
...
**Actualización 2026-04-14 noche — Sprint A2 Fase 2B cerrada** (commit `e06985d`, pusheado a origin/main). Las 120 sentencias de Jurisprudencia EUREKA **están en Supabase producción**. Pipeline en 3 pasadas + reparador del grafo: Pasada 0 pre-carga dict de normas existentes desde Fase 2A, Pasada 1 inserta `jurisprudence_sources` + `eureka_sources_metadata` con embeddings OpenAI (120 sentencias, 872 resolved + 287 unresolved de `concordances` desde sentencias), Pasada 3 REPARADOR promueve los **171 links Normativa→Jurisprudencia** que quedaron huérfanos en Fase 2A (100% cobertura, 171/171). **Costo OpenAI real**: $0.005134. **Tiempo**: 333s (5m33s). Después de Fase 2B: total `concordances` = **2.928** (1.769 desde normas + 1.159 desde sentencias), de los cuales **2.366 resolved** (80.8% del grafo — los 562 unresolved apuntan a docs fuera del corpus EUREKA actual).

**Actualización 2026-04-14 noche — Sprint A2 Fase 2D-suin cerrada** (commit `e5aca67` bumpeando a **v3.9.0**, pusheado a origin/main). **Scraper de SUIN-Juriscol completo**: el 93.7% de los 270 docs EUREKA Normativa linkea a SUIN (239 docs), que expone HTML legacy ASP sin API. Script `scrape_suin_to_normative_articles.py` (933 líneas, 4 pasadas con embedding inline — Opción C) scrapea esos 239 docs, extrae texto por artículo con regex tolerante a `ARTíCULO` con í minúscula (fix SA-DEUDA-7 aplicado localmente), deduplica modificaciones editoriales repetidas, y ejecuta INSERT atómico con embedding OpenAI ya generado. **239/239 normas scrapeadas, 7.758 artículos con embedding (100%)**, 239 `normative_sources` enriquecidas con issue_date/issuing_body/publication_source/vigencia extraídas del HTML SUIN. Una norma (Ley 2454/2025 "Ley Lorenzo") falló por timeout de red en primer intento (26 artículos); recuperada en un segundo run con `--only-missing`. **Hallazgo crítico del scraper**: SUIN tiene WAF que bloquea User-Agents con `Bot` en el nombre — se usó `VIGIA-Ingest/1.0` sin `Bot`. **Costo OpenAI real total**: $0.048737 (2.4M tokens). **Tiempo**: 32 min. Fingerprints SHA-256 por doc guardados en `scripts/eureka/suin_scrape_fingerprints.json` para monitoreo futuro de cambios en SUIN (Fase 3).

**Corpus en Supabase post Fase 2D-suin (estado actual)**:
- `normative_sources`: **288** (18 Sprint A + 270 EUREKA)
- `normative_articles`: **11.365** con embedding (3.607 Sprint A + 7.758 SUIN — **+214% vs corpus original Sprint A**)
- `jurisprudence_sources`: **120** sentencias
- `eureka_sources_metadata`: **390** (270 normas + 120 sentencias) con resumen curado + embedding editorial
- `concordances`: **2.928** (2.366 resolved, 80.8%)

**Costo OpenAI acumulado de todas las fases EUREKA**: ~$0.062 (<7 centavos para vectorizar 11.755 chunks embedables).

**Correcciones al handoff anterior** (detectadas en esta sesión):
- `vercel.json` SPA routing: **✅ ya estaba cerrado desde `df2f2f0` v3.1.1** (no era deuda pendiente).
- RLS en `instruments`/`obligations`/`documents`/`regulatory_alerts`: **✅ ya estaba cerrado** con policies usando `is_org_member(org_id)` (para las 3 primeras) y policy `qual=true` universal para `regulatory_alerts` (diseño intencional — alertas son globales, no por org). El handoff §6 ya lo marcaba "✅ 8 tablas con is_org_member()" — confirmado en sesión.

**Deudas técnicas pendientes REALES** (bugs que sí requieren acción):
- **SA-DEUDA-7** sigue vigente: el regex en `supabase/functions/norm-ingest/index.ts` carece de flag `/i` y `[íÍiI]culo` requiere "culo" minúsculas. Cuando pypdf extrae "ARTíCULO" (í minúscula en medio de mayúsculas), la edge TypeScript no matchea. En la ingesta del Sprint A algunas normas pueden haber perdido artículos. El scraper SUIN local ya aplica `re.IGNORECASE` correcto. **Cerrar en Fase 2C** junto con la actualización del chat-bot para usar el corpus ampliado.
- **Fase 2C** (pendiente): extender `chat-bot` para incluir `eureka_sources_metadata.resumen_embedding` en el RAG, mostrar citas con concordancias, filtros por corte/tipo/año. Requiere briefing separado.
- **Fase 2D-other** (pendiente): 30 docs Normativa no-SUIN (minambiente=9, un.org=3, alcaldiabogota=2, funcionpublica=2, etc.) — cada host con su propio parser HTML. Impacto menor (~10% docs adicionales).
- Fases 1C/1D/1E (opcionales): Procedimientos + Manuales / Conceptos Jurídicos / Documentos Estratégicos + Gestión. Ver §13bis.

Siguiente hito recomendado: **Sprint A2 Fase 2C** (UX del bot con corpus ampliado + fix SA-DEUDA-7) o **Sprint A2 Fase 2D-other** (cerrar los 30 docs no-SUIN). **Sprint B** sigue pendiente con briefing propio (ver §15).
...
### Falsos positivos cerrados (verificado 2026-04-14 noche)

Items que a veces aparecen como "deuda" pero ya están cerrados:

- ✅ **`vercel.json` SPA routing**: ya existe desde commit `df2f2f0` (v3.1.1). Contenido: `{"rewrites":[{"source":"/(.*)","destination":"/index.html"}]}`. No es deuda.
- ✅ **RLS en `instruments`/`obligations`/`documents`/`regulatory_alerts`**: ya activo. Policies existentes: `is_org_member(org_id)` para las 3 primeras (SELECT y ALL separadas), policy `qual=true` para `regulatory_alerts` (diseño intencional — alertas son globales, no por org; por eso la tabla no tiene `org_id`). Confirmado en sesión 2026-04-14 noche cuando una migration hand-rolled falló y el rollback atómico mostró el estado pre-existente. No es deuda.
...
**Fases 1A, 1B, 2A, 2B y 2D-suin CERRADAS**. Pendientes reales: Fase 2C (UX bot + SA-DEUDA-7) y Fase 2D-other (30 docs no-SUIN). Ver §13bis para el plan de categorías EUREKA restantes.
...
| **A2.F2B** | **Ingesta a Supabase de Jurisprudencia EUREKA (120 sentencias) + script reparador 171 links** | ✅ **Cerrado 2026-04-14** (commit `e06985d`) |
| **A2.F2D-suin** | **Scraper SUIN-Juriscol: 239 normas EUREKA externas → 7.758 artículos con embedding en `normative_articles`** | ✅ **Cerrado 2026-04-14 — v3.9.0** (commit `e5aca67`) |
...
| A2.5 | Expandir corpus con 10-20 normas adicionales (compensaciones, SINAP, jurisprudencia clave) | ✅ Cubierto sustancialmente por EUREKA Fases 2A+2B+2D-suin (+7.758 artículos) |
| **A2.F2C** | **UX del bot con corpus editorial EUREKA** (RAG extendido sobre `eureka_sources_metadata.resumen_embedding` + `normative_articles` nuevos 7.758 + citas con `concordances`) + **fix SA-DEUDA-7 en `norm-ingest/index.ts`** | **Abierto — siguiente recomendado, briefing separado** |
| **A2.F2D-other** | **Scrapers para los 30 docs Normativa no-SUIN** (minambiente=9, un.org=3, alcaldiabogota=2, funcionpublica=2, etc.) — cada host con parser propio | **Abierto — siguiente tras 2C** |
...
- **Sprint A2 Fase 2C** (UX del bot con corpus ampliado):
  - Extender `chat-bot` para usar los nuevos 7.758 artículos SUIN
    (ya están en `normative_articles` con embedding). Automáticamente
    cubiertos por la función RPC `match_normative_articles` vía
    `filter_scope` si se pasa NULL.
  - Integrar `eureka_sources_metadata.resumen_embedding` en una
    nueva RPC o extensión del RAG — hoy el bot no lo consulta.
  - Mostrar concordancias en las respuestas (navegación del grafo).
  - **Cerrar SA-DEUDA-7 en la edge function `norm-ingest/index.ts`**
    agregando flag `/i` al regex de artículos y re-ingerir las normas
    del corpus original Sprint A potencialmente afectadas.
  - Requiere briefing separado (decisiones de UX + RAG shape).
- **Sprint A2 Fase 2D-other** (30 docs Normativa no-SUIN):
  minambiente=9, un.org=3, alcaldiabogota=2, funcionpublica=2,
  cancilleria=1, ica=1, sisjur=1, otros. Cada host con su propio
  parser HTML. Impacto marginal (<10% del corpus). Puede ir antes
  o después de Fase 2C.
...
- **Sprint A2 Fase 1C** (EUREKA: Procedimientos + Manuales) —
  ~80-200 docs, pipeline reutilizable. Ver §13bis.
...
2. `git log --oneline | head -14` debe empezar con el commit del handoff y luego `e5aca67 v3.9.0 - Fase 2D-suin COMPLETA…`, `e06985d fase-2b: ingesta 120 sentencias EUREKA…`, `863d711 docs: cierre Fase 2A…`, `476310c fase-2a: ingesta 270 normas EUREKA…`, `2cd085d v3.8.0 - seed datos demo…` (push externo), `df402b3 docs: actualizar handoff con cierre de sprint a2 fase 1b`, `247ed7c sprint a2 fase 1b: jurisprudencia eureka…`, `4201481 docs…`, `47b63e6 sprint a2 fase 1a…`, `d389e4d docs…`, `ae035f8 chore: security hardening…`, `23021a6 chore: versionar 12 edge functions…`, `69dd0dd v3.7.0`, `4e206fe v3.6.1`.
3. Con MCP Supabase, verificar estado post Fase 2D-suin: `SELECT count(*) FROM normative_sources WHERE corpus_source='sprint_a_corpus'` = **18**. `SELECT count(*) FROM normative_sources WHERE corpus_source='eureka_metadata'` = **270**. `SELECT count(*) FROM normative_articles WHERE embedding IS NOT NULL` = **11.365** (3.607 Sprint A + 7.758 SUIN). `SELECT count(*) FROM eureka_sources_metadata WHERE resumen_embedding IS NOT NULL` = **390**. `SELECT count(*) FROM concordances WHERE resolved=true` = **2.366**. `SELECT count(*) FROM jurisprudence_sources` = **120**. `SELECT count(*) FROM normative_sources WHERE corpus_source='eureka_metadata' AND parser_method='regex'` = **239** (las enriquecidas por scraper SUIN).
...
**Fin del handoff — cierre 2026-04-13 + Sprint A2 completo (Fases 1A, 1B, 2A, 2B, 2D-suin, 2D-inline-anla, 2D-minambiente-pdf, 2D-minambiente-html, 2D-sisjur, 2D-funcionpublica, 2D-outliers) + chat-bot v7 con REGLA 13 ENARA. v3.9.1 en producción.**

---

## Fase 2D completa — Cerrada 2026-04-14 noche-tarde (v3.9.1)

### Subfases ejecutadas

| Subfase | Docs | Artículos | Costo OpenAI | Commit |
|---|---:|---:|---:|---|
| 2D-inline-anla (PDFs ya descargados Fase 1A) | 11 | 111 | $0.0013 | `50d6d43` |
| 2D-minambiente-pdf (URL directa wp-content) | 5 | 47 | $0.0005 | `9f6f674` |
| 2D-minambiente-html (carátula → PDF resuelto) | 4 | 69 | $0.0005 | `8650ba4` |
| 2D-sisjur (Norma1.jsp Windows-1252) | 3 | 58 | $0.0007 | `8650ba4` |
| 2D-funcionpublica (eva/gestornormativo) | 2 | 7 | $0.00004 | `<this>` |
| 2D-outliers (un.org + ica + cancilleria) | 4/5 | 4 | $0.0003 | `<this>` |
| **Total Fase 2D-other** | **29/30** | **296** | **$0.003** | — |
...
| jurisprudence_sources | 120 |
| **normative_articles con embedding** | **11.661** ✓ |
| eureka_sources_metadata con resumen_embedding | 390 |
| concordances | 2.928 (2.366 resolved) |

**+223% vs corpus Sprint A original** (3.607 → 11.661 artículos vectorizados).

### Costo OpenAI acumulado todas las fases EUREKA

- Fase 2A (270 normas metadata): $0.0085
- Fase 2B (120 sentencias metadata + reparador): $0.0051
- Fase 2D-suin (239 normas × art.): $0.0487
- Fase 2D-inline-anla (11 docs): $0.0013
- Fase 2D-minambiente-pdf (5 docs): $0.0005
- Fase 2D-minambiente-html (4 docs): $0.0005
- Fase 2D-sisjur (3 docs): $0.0007
- Fase 2D-funcionpublica (2 docs): $0.00004
- Fase 2D-outliers (4 docs): $0.0003
- **TOTAL: ~$0.067** (menos de 7 centavos para 11.661 artículos + 390 resúmenes vectorizados)

### REGLA 13 ENARA — chat-bot v7 deployed

Commit `03b3e0a` agregó al system prompt del chat-bot la invitación directa a contactar ENARA Consulting cuando la respuesta requiera asesoría especializada. **Deployed via MCP a Supabase producción** (chat-bot v7 ACTIVE, id `f9fa02a6`). Las próximas respuestas del bot que activen REGLA 13 cierran con bloque de contacto:
- 📧 info@enaraconsulting.com.co
- 🌐 www.enaraconsulting.com.co
- 📞 +57 314 330 4008 / +57 320 277 3972

Reemplaza explícitamente cualquier "consulta con asesor externo" genérico de REGLA 11.

### corpus_gaps.json — registro de docs no procesados

Creado en commit `abaf2a2`, actualizado al cierre del día con 7 entries:

| Slug abreviado | Razón | Status | Prioridad |
|---|---|---|---|
| Res 631/2015 (Vertimientos) | scan_no_ocr | pendiente | **alta** |
| Res 2086/2010 (Tasación Multas) | scan_no_ocr | pendiente | alta |
| Res 108/2015 (Formato Licencia) | scan_no_ocr | pendiente | media |
| Res 827/2018 (Sancionatorio) | scan_no_ocr | pendiente | media |
| Res 762/2022 (Fuentes móviles) | scan_no_ocr | pendiente | alta |
| Decreto 43/2024 (Reservas) | no_url | **skip_definitivo** | baja |
| Directiva Pres 10/2013 (Consulta Previa) | ssl_obsoleto_servidor cancilleria | pendiente_ssl_workaround | media |

5 candidatos principales para Fase 2C (OCR sistemático con Claude vision). Patrón Sprint A: ~$0.04-$0.15 por scan via edge function `norm-extract-text`. Total proyectado para los 5: ~$0.30-$0.50.

### Próximas fases

- **Fase 2C** (siguiente recomendado): integrar el corpus ampliado en UX del bot (RAG sobre `eureka_sources_metadata.resumen_embedding` + citas de concordances + filtros UI por corte/tipo/año) + OCR de los 5 scans pendientes + fix SA-DEUDA-7 en `norm-ingest`. Briefing separado.
- Fases 1C/1D/1E (opcionales): otras 5 categorías EUREKA (Procedimientos, Manuales, Conceptos Jurídicos, Documentos Estratégicos, Gestión del Conocimiento).

---

## Fase 2B — Cerrada 2026-04-14 noche

### Qué se hizo
- Ingesta de 120 sentencias Jurisprudencia EUREKA a `jurisprudence_sources`
- 120 filas en `eureka_sources_metadata` con `source_type='sentencia'` + `resumen_embedding`
- 1.159 concordances desde sentencias (872 resolved: 446 → norma, 426 → sentencia; 287 unresolved)
- Reparador automático: 171 links Normativa→Jurisprudencia promovidos de unresolved a resolved (100% cobertura)
- Commit: `e06985d`

### Números clave
- Costo OpenAI real: $0.005134
- Elapsed: 333s (5 min 33s)
- Post-run: `concordances` totales 2.928, resolved 2.366 (80.8%)

---

## Fase 2D-suin — Cerrada 2026-04-14 noche (v3.9.0)

### Qué se hizo
- Scraper de SUIN-Juriscol: `scripts/eureka/scrape_suin_to_normative_articles.py` (933 líneas, 4 pasadas con Opción C = embedding inline)
- 239/239 normas SUIN scrapeadas (el 93.7% de Normativa EUREKA que vive en sitios externos)
- 7.758 artículos con embedding inline, 100% cobertura (reparado en 2 corridas: 7.732 primera + 26 con `--only-missing`)
- 239 `normative_sources` enriquecidas con metadata extraída del HTML SUIN (issue_date, issuing_body, publication_source, vigencia, parser_quality)
- Fingerprints SHA-256 por doc en `scripts/eureka/suin_scrape_fingerprints.json` para monitoreo de cambios (Fase 3)
- Fix SA-DEUDA-7 aplicado localmente en el scraper (regex con `re.IGNORECASE`). La edge function `norm-ingest/index.ts` sigue con el bug — se cierra en Fase 2C.
- Commit: `e5aca67` (v3.9.0)

### Números clave
- Costo OpenAI real: $0.048737 (2.4M tokens, 239 docs × ~32 art avg)
- Elapsed: 32 min total (31.4 min primer run + 11s recovery run)
- Post-run: `normative_articles` totales **11.365** (+214% vs corpus Sprint A original)

### Hallazgo operativo (guardar para scrapers futuros)
**SUIN tiene WAF que bloquea User-Agents con `Bot` en el nombre**. Detectado en primer smoke test (500 Internal Server Error con UA `VIGIAResearchBot/1.0`). Fix: usar `VIGIA-Ingest/1.0` (sin "Bot"). Otros dominios colombianos oficiales pueden tener el mismo patrón.

### Estado de la DB post-Fase 2D-suin
| tabla | filas |
|-------|-------|
| normative_sources (sprint_a_corpus) | 18 |
| normative_sources (eureka_metadata) | 270 |
| jurisprudence_sources | 120 |
| normative_articles con embedding | 11.365 |
| eureka_sources_metadata con resumen_embedding | 390 |
| concordances | 2.928 (2.366 resolved, 562 unresolved) |

### Próximas fases
- **Fase 2C**: extender `chat-bot` para RAG sobre el corpus ampliado + fix SA-DEUDA-7 en `norm-ingest`. Briefing separado pendiente.
- **Fase 2D-other**: 30 docs no-SUIN (minambiente, un.org, alcaldiabogota, funcionpublica, etc.). Parser HTML por host. Impacto marginal (<10%).
- Fases 1C/1D/1E (opcionales): otras 5 categorías EUREKA (Procedimientos, Manuales, Conceptos Jurídicos, Documentos Estratégicos, Gestión del Conocimiento).
