#!/usr/bin/env python3
"""Sprint A2 Fase 2D-funcionpublica — ingesta de 2 decretos publicados en
funcionpublica.gov.co backend gestornormativo (norma.php?i=X).

Container: <main class="main">. Charset UTF-8 (a diferencia de sisjur).
Articulado embebido como texto plano con formato "ARTÍCULO\n N.\n ...".
"""
...
REPORT_JSON = HERE / "ingest_funcionpublica_report.json"
...
def parse_articles(text):
    ...
    chapters = [(m.start(), m.group(0).strip()) for m in CHAPTER_RE.finditer("\n" + norm)]
    def chap_at(p):
        last = None
        for i, l in chapters:
            if i < p: last = l
...
def fetch_body_text(url):
    ...
    container = soup.select_one("main.main") or soup.find("main") or soup.find("body") or soup
...
def main():
    ...
    candidates = [r for r in data["records"]
                  if "funcionpublica.gov.co/eva/gestornormativo/" in (r.get("primary_source_url") or "").lower()]
    print(f"[info] Función Pública gestornormativo: {len(candidates)}")
    ...
    stats = {"started_at": time.strftime("%Y-%m-%dT%H:%M:%S%z"), "dry_run": args.dry_run,
             ...
             "fetch_failed": 0, "articles_parsed_raw": 0, "articles_dedup": 0,
    ...
    print(f"\n[pasada 1] Fetch HTML + regex + embed + INSERT (delay {FETCH_DELAY}s)…")
    t0 = time.time()
    for idx, r in enumerate(resolved, 1):
        ...
        if not arts and len(text) >= 200:
            arts = [{"article_number": None, "article_label": "Documento completo", "title": None,
                     "content": text, "chapter": None, "order_index": 1,
        ...
        stats["per_doc"].append({"slug": slug, "chars": len(text), "raw": len(raw),
                                 "final": len(arts), "quality": pq})
    ...
    for k in ["total_candidates","total_resolved","fetch_failed","articles_parsed_raw",
              "articles_dedup","articles_inserted","embeddings_generated","norms_patched",
